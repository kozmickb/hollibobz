...
import { incrUsage, recordProviderUsage } from "../../db";
import { attachSubjectId, getSubjectId } from "../../middleware/subject";
import { getPlan, checkMonthlyUsageLimit, LIMIT_RULES } from "../../middleware/entitlement";
...
airportsRouter.get("/:iata/schedule", async (req, res, next) => {
    try {
        ...
        const safeDuration = Math.min(Number(durationMinutes !== null && durationMinutes !== void 0 ? durationMinutes : maxDuration), maxDuration);
        console.log('🔍 Airport schedule request:', {
            iata,
            offsetMinutes,
            requestedDuration: durationMinutes,
            safeDuration,
            direction: direction !== null && direction !== void 0 ? direction : "Both",
            subjectId,
            plan
        });
    ...
    try {
        ...
        if (flights.length === 0 && source !== 'aviationstack') {
            ...
            try {
                ...
                const relevantFlights = scheduleResult.departures.filter((flight) => flight.arrive.iata === destinationCode);
